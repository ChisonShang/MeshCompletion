...
for indPlane = 1: modelPlane.numPlane
    ...
    %uvPlaneIndCur  = logical(ones(size(uvPlaneIDData)));
    ...
    %%
    costDirect1 = min(costDirect/optS.imgSize, optS.directThres);
    ...
    if(a==0||c==0)
        for i=1:num
            ...
            x1 = trgPos(i,1);
            y1 = trgPos(i,2);
            temp = (x2-x1)/(sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))) + (y2-y1)/(sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)));
            temp2 = abs(temp);
            costDirect(i,1) = log(temp2);
        end
    else
        k1 = b/a;
        k2 = d/c;
        for i=1:num
            ...
            x1 = trgPos(i,1);
            y1 = trgPos(i,2);
            temp = abs((x2-x1+k1*y2-k1*y1)/(sqrt(1+k1*k1)*sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)))) + abs((x2-x1+k2*y2-k2*y1)/(sqrt(1+k2*k2)*sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))));
            temp2 = abs(temp);
            costDirect(i,1) = log(temp2);
        end
    end
    tt= costDirect(:,1);
    tttt = mapminmax(tt.',-optS.directThres/optS.imgSize, 0);
    if(isequal(tttt, tt.'))
        tttt= zeros(size(tttt));
    end
    costDirect = min(costDirect1 +tttt.', [], 2);
end