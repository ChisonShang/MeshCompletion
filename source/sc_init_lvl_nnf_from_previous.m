% SC_INIT_LVL_NNF
function [img, NNF] = sc_init_lvl_nnf_from_previous(img, imgp, NNF, holeMask, modelPlaneCur, modelRegCur, optS)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[w, h,~] = size(img);
imagep = imresize(imgp,[w, h],'nearest');
%imwrite(imagep,'temp.png');
%imgD = imread('temp.png');
bw=rgb2gray(imagep);
f = edge(double(bw),'prewitt',12,'both');
if(optS.iLvl == optS.numPyrLvl)
    % Initialize the NNF for the coarest level using random sampling
    NNF = sc_init_nnf(holeMask, modelPlaneCur, modelRegCur, optS, f);
  else
    % Initialize the NNF upsampling of NNF from previous level
    NNF = sc_upsample(holeMask, NNF, modelPlaneCur, modelRegCur, optS);
    trgPatch = sc_prep_target_patch_include_edge_sort_all_level(img, NNF.uvPix.sub, optS, f);
...
% SC_INIT_NNF
function NNF = sc_init_nnf(holeMask, modelPlane, modelReg, optS, edge)
...
% Initialize indMap
% =========================================================================
NNF.trgPatchInd = getTargPathIndEdge(NNF, optS, edge);
...
if(NNF.validPix.numPix)
    ...
    %randInd   = round((NNF.imgW/2)*ones(NNF.uvPix.numPix, 1));
...
function trgPatchInd = getTargPathIndEdge(NNF, optS, edge)

pixIndMap = single(reshape(1:NNF.imgH*NNF.imgW, NNF.imgH, NNF.imgW));
indTrgPatch   = sc_prep_target_patch_include_edge_sort_first(pixIndMap, NNF.uvPix.sub, optS, edge);
