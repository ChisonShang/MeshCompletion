...
for i = 1:optS.numPassPerIter
    ...
    [NNF, n] = sc_constrained_random_search(trgPatch, img, NNF, modelPlane, optS);
...
function [NNF, nUpdateTotal] = sc_constrained_random_search(trgPatch, img, NNF, modelPlane, optS)
...
while(searchRad > 1)
    ...
    trgPos = NNF.uvPix.sub;
    [wCand,~] = size(srcPos);
    srcPosConstrained = srcPos;
    for i = 1:wCand
        trgPosTemp = trgPos(i,:);
        srcPosTemp = srcPosConstrained(i,:);
        [xTemp,yTemp] = constrained_expansion(optS.direction, searchRad,trgPosTemp, srcPosTemp, optS);
        srcPosConstrained(i,1) = xTemp;
        srcPosConstrained(i,2) = yTemp;
    end
...
function [NNF, nUpdateTotal] = sc_random_constrained_search(trgPatch, img, NNF, modelPlane, optS)
